...
class Snake:
    ...
    def __init__(self, game_map, init_direc=None, init_bodies=None, init_types=None):
        ...
        Args:
            game_map (base.map.Map): 蛇移动的地图
            init_direc (base.direc.Direc): 初始方向
            init_bodies (list of base.pos.Pos): 初始蛇身体位置
            init_types (list of base.point.PointType): init_bodies中每个位置的类型
        """
        self._map = game_map
    ...
    def reset(self, reset_map=True):
        rand_init = False
        if self._init_direc is None:  # 随机初始化
            rand_init = True
